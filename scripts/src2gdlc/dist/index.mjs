// index.js
import { readdirSync, lstatSync, statSync, existsSync, writeFileSync, readFileSync as readFileSync3, renameSync, unlinkSync } from "fs";
import { basename, dirname as dirname2, join, relative, resolve as resolve2, extname } from "path";
...
function formatGdlcV2(files, options) {
  const lines = [];
...
function normalizeDescription(text) {
  if (!text) return "";
  let cleaned = text.replace(/^\/\*\*?\s*|\s*\*\/$/g, "").replace(/^\s*\/\/\/?\s*/gm, "").replace(/^\s*\*\s?/gm, "").trim();
  if (!cleaned) return "";
...
var WASM_DIR = resolve(__dirname, "grammars");
var initPromise = null;
var languageCache = /* @__PURE__ */ new Map();
async function ensureInit() {
  if (!initPromise) {
    initPromise = Parser.init().catch((err2) => {
      initPromise = null;
      throw new Error(`WASM runtime init failed: ${err2.message}`);
    });
  }
  return initPromise;
}
async function loadLanguage(wasmFile) {
  if (!languageCache.has(wasmFile)) {
    ...
    languageCache.set(wasmFile, Language.load(wasmPath).catch((err2) => {
      languageCache.delete(wasmFile);
      throw new Error(`Failed to load grammar '${wasmFile}': ${err2.message}`);
    }));
  }
  return languageCache.get(wasmFile);
}
async function extractFile(filePath, mapping, options = {}) {
  await ensureInit();
  ...
  let source = readFileSync2(filePath, "utf-8");
  ...
  try {
    ...
    console.error(`Warning: parse failed for ${filePath}: ${err2.message}`);
    return { exports: [], imports: [], docstring: "" };
  }
...
var python_default = {
  ...
  getExports(node) {
    ...
    if (target.type === "function_definition" || target.type === "class_definition") {
      ...
      if (name2.startsWith("_")) return null;
      return name2;
    }
    return null;
  },
...
// mappings/go.js
function isCapitalized(name2) {
  if (!name2) return false;
  const first = name2.charAt(0);
...
function hasVisibilityModifier(node) {
  for (let i2 = 0; i2 < node.childCount; i2++) {
    if (node.child(i2).type === "visibility_modifier") return true;
  }
  return false;
}
function getFirstIdentifier(node) {
  ...
  if (node.type === "scoped_identifier") {
    const path2 = node.childForFieldName("path");
    if (path2) return getFirstIdentifier(path2);
    const name2 = node.childForFieldName("name");
    return name2 ? name2.text : "";
  }
  ...
  if (node.type === "use_wildcard") {
    for (let i2 = 0; i2 < node.namedChildCount; i2++) {
      const result = getFirstIdentifier(node.namedChild(i2));
...
var javascript_default = {
  ...
  getExports(node) {
    if (node.type === "export_statement") {
      const names = [];
      const decl = node.namedChildren.find(
        (c) => ["function_declaration", "class_declaration", "lexical_declaration"].includes(c.type)
      );
      if (decl) {
        if (decl.type === "lexical_declaration") {
          for (const declarator of decl.namedChildren) {
  ...
  getImportModule(node) {
    ...
    if (node.type === "expression_statement" || node.type === "lexical_declaration" || node.type === "variable_declaration") {
      ...
      for (const call of calls) {
        ...
        if (!args2) continue;
        const strNode = args2.descendantsOfType?.("string")[0];
        if (!strNode) continue;
        const raw = strNode.text.replace(/['"]/g, "");
        if (raw.startsWith(".")) {
          const parts2 = raw.split("/");
          modules.push(parts2[parts2.length - 1]);
...
var csharp_default = {
  ...
  getExports(node) {
    switch (node.type) {
      ...
      case "struct_declaration": {
        if (!hasPublicModifier2(node)) return null;
        const name2 = node.childForFieldName("name");
        return name2 ? name2.text : null;
      }
  ...
  getImportModule(node) {
    ...
    const qname = node.descendantsOfType("qualified_name")[0] || node.descendantsOfType("identifier")[0];
  ...
  getDocText(node) {
    ...
    const cleaned = text.split("\n").filter((l) => l.trimStart().startsWith("///")).map((l) => l.replace(/^\s*\/\/\/\s?/, "")).join(" ").replace(/<\/?summary>/g, "").replace(/<[^>]+>/g, "").trim();
    if (!cleaned) return "";
    const firstSentence = cleaned.match(/^[^.!?]+[.!?]?/)?.[0] || cleaned;
    return firstSentence.trim();
  }
};
function hasPublicModifier2(node) {
  for (let i2 = 0; i2 < node.namedChildCount; i2++) {
...
var c_default = {
  ...
  getImportModule(node) {
    ...
    const pathNode = node.childForFieldName("path") || node.descendantsOfType("system_lib_string")[0] || node.descendantsOfType("string_literal")[0];
...
var ruby_default = {
  ...
  getExports(node) {
    ...
    if (node.type === "method") {
      ...
      if (!nameNode) return null;
      const name2 = nameNode.text;
      if (name2 === "initialize") return null;
      return name2;
    }
  ...
  getImportModule(node) {
    ...
    const strNode = args2.descendantsOfType("string_content")[0];
    if (!strNode) return null;
    const parts2 = strNode.text.split("/");
    return parts2[parts2.length - 1];
  },
  isDocComment(node) {
    return node.type === "comment" && node.text.trimStart().startsWith("#");
...
var php_default = {
  ...
  langId: "php",
  getExports(node) {
    if (node.type === "class_declaration" || node.type === "interface_declaration" || node.type === "enum_declaration" || node.type === "trait_declaration") {
      const nameNode = node.childForFieldName("name");
      return nameNode ? nameNode.text : null;
    }
    if (node.type === "function_definition") {
  ...
  getImportModule(node) {
    ...
    for (const clause of clauses) {
      const qname = clause.descendantsOfType("qualified_name")[0] || clause.descendantsOfType("name")[0];
      if (qname) {
        const fullName = qname.text;
        const parts2 = fullName.split("\\");
        const imported = parts2[parts2.length - 1];
...
var swift_default = {
  ...
  getExports(node) {
    ...
    if (["class_declaration", "protocol_declaration", "struct_declaration", "enum_declaration"].includes(node.type)) {
      if (isPrivateOrFileprivate(node)) return null;
      const nameNode = node.childForFieldName("name") || node.namedChildren.find((c) => c.type === "type_identifier") || node.namedChildren.find((c) => c.type === "identifier");
      const name2 = nameNode ? nameNode.text : null;
      if (!name2 || node.namedChildren.some((c) => c.type === "ERROR")) {
        return extractNameFromText(node) || name2;
      }
      return name2;
    }
    return null;
  ...
  getDocText(node) {
    ...
    const cleaned = docLines.map((l) => l.replace(/^\s*\/\/\/\s?/, "")).join(" ").trim();
    if (!cleaned) return "";
    const firstSentence = cleaned.match(/^[^.!?]+[.!?]?/)?.[0] || cleaned;
    return firstSentence.trim();
  }
...
// index.js
var mappings = [typescript_default, python_default, go_default, java_default, rust_default, javascript_default, csharp_default, c_default, cpp_default, ruby_default, php_default, kotlin_default, swift_default, bash_default];
...
function findSourceFiles(dir, recursive, langFilter) {
  const supported = langFilter ? mappings.filter((m) => m.name === langFilter).flatMap((m) => m.extensions) : allExtensions();
  ...
  try {
    ...
    console.error(`Warning: cannot read directory ${dir}: ${err2.message}`);
    return results;
  }
  for (const entry of entries) {
    ...
    try {
      ...
      console.error(`Warning: cannot stat ${full}: ${err2.message}`);
      continue;
    }
...
async function cli(args2) {
  ...
  for (const flag of RESERVED_FLAGS) {
    if (args2.includes(flag) || args2.some((a) => a.startsWith(flag + "="))) {
  ...
  const flags2 = Object.fromEntries(
    args2.filter((a) => a.startsWith("--")).map((a) => {
      const eq = a.indexOf("=");
  ...
  const outputFile = flags2.output;
  const recursive = "recursive" in flags2;
  const langFilter = flags2.lang;
  const ignoreFileFlag = flags2["ignore-file"];
  ...
  if (stat.isDirectory()) {
    try {
      ...
      ignoreSearchDir = resolve2(target);
    }
  } else {
    try {
      ignoreSearchDir = execSync("git rev-parse --show-toplevel", {
  ...
  if (ignorePatterns.length > 0) {
    const baseDir2 = ignoreSearchDir;
    const before = files.length;
    files = files.filter((f) => {
      const rel = relative(baseDir2, resolve2(f));
  ...
  if (files.length === 0) {
    if (outputFile) {
      const output2 = formatGdlcV2([], { sourceHash: "0000000", generated: today });
      atomicWrite(outputFile, output2);
  ...
  const baseDir = stat.isDirectory() ? resolve2(target) : dirname2(resolve2(target));
  const fileResults = [];
  for (const file of files) {
    ...
    try {
      ...
      console.error(`Error processing ${file}: ${err2.message}`);
      continue;
    }
    let langId = mapping.langId;
    if (ext === ".tsx") langId = "tsx";
    else if (ext === ".jsx") langId = "jsx";
    const relPath = relative(baseDir, resolve2(file));
    fileResults.push({
      path: relPath,
      lang: langId,
  ...
  const sourceHash = computeSourceHash(files.map((f) => resolve2(f)));
...
var isMainModule = process.argv[1] && resolve2(process.argv[1]) === resolve2(fileURLToPath2(import.meta.url));
if (isMainModule && process.argv.length > 2) {
  cli(process.argv.slice(2)).catch((err2) => {
